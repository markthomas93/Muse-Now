...
class ParWords: ParStr {

    var recents = NodeRecents()
    var words : [Substring]!
    ...
    func update(_ str_: String,_ time_: TimeInterval = Date().timeIntervalSince1970) {
    ...
    //? Advance past match and return nodeAny with number of hops from normal sequence.
    override func advancePar(_ node:Node!, _ index:Int, _ str: String!,_ deltaTime:TimeInterval = 0) -> NodeAny! {

        /// matching a recent query is treated as a last resort, which is insured by adding cuttoff time for short term memory
        let penaltyHops = deltaTime > 0 ? Int(deltaTime + NodeRecents.shortTermMemory) : 0
        ...
        let nodeAny = NodeAny(node, str, hops + penaltyHops, Date().timeIntervalSince1970)
        recents.add(nodeAny)
        return nodeAny
    }
    ...
    override func matchMatchStr(_ node:Node!) -> NodeAny! {
        ...
        if recents.array.count > 0,
          node.reps.repMin >= 1 {
            ...
            for nodeAny in recents.array.reversed() {
                if let nodeAny = nodeAny,
                    let id = nodeAny.node?.id, id == node.id,
                    let word = nodeAny.any as? String {

                    let deltaTime = time - nodeAny.time
    ...
    override func matchQuote(_ node:Node!, withEmpty:Bool=false) -> NodeAny! {
        ...
        if node.pattern == "" { return withEmpty ? NodeAny(node,"") : nil }
    ...
    override func matchRegx(_ node:Node!) -> NodeAny! {
        ...
        func match(_ i: Int) -> String! {
            ...
            if let rangeRegx = matchRegxWord(node!.regx!,word) {
                return String(str[rangeRegx.matching])
            }
            else {
                return nil
            }
